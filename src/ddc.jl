...
function ddcn(signal::Vector{T},n::Integer) where {T<:Real}
    ...
    @inbounds for i in 1:length(signal)
        out[1,i] = X[1,i%n]*signal[i]
        out[2,i] = X[2,i%n]*signal[i]
    end
	return out
end
...
function reshape_IQ_seg(signal::Matrix{T},seg_len::Integer) where {T<:Real}
